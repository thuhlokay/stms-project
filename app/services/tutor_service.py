...
class TutorService:
    ...
    async def create_profile(self, user_id: int, tutor_in: TutorCreate,
                              student_card: UploadFile, transcript: UploadFile,
                              profile_picture: UploadFile = None) -> Tutor:
        ...
        if student_card.content_type not in ALLOWED_IMAGE_TYPES:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                                detail="Student card must be JPEG or PNG")
        if transcript.content_type not in ALLOWED_DOC_TYPES:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                                detail="Transcript must be PDF, JPEG or PNG")
        ...
        if profile_picture:
            if profile_picture.content_type not in ALLOWED_IMAGE_TYPES:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                                    detail="Profile picture must be JPEG or PNG")
            profile_picture_path = save_upload(profile_picture, "profile_pictures")

        return await self.repo.create(
            user_id=user_id, tutor_in=tutor_in,
    ...
    async def list_tutors(self, skip: int = 0, limit: int = 100) -> list[Tutor]:
